...
public class TransactionsService {
  ...
  //  • Adding a user
  User addUser(final String newName, final int newBalance) throws Exception {
    User user = new User(newName, newBalance);
    users.addUser(user);
    return user;
  }
  ...
  Transaction removeTransfer(final int id, final UUID tid) throws Exception {
    return users.getUserById(id).getTransactions().removeById(tid);
  }
